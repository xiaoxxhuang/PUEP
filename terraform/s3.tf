...
# Bucket for storing images, publicly accessible
resource "aws_s3_bucket" "puep_images_s3_bucket" {
  bucket = "${var.project_name}-${local.environment}-images-s3-bucket"
}

resource "aws_s3_bucket_acl" "puep_images_s3_bucket_acl" {
  bucket = aws_s3_bucket.puep_website_s3_bucket.id
  acl    = var.acl_value
}

resource "aws_s3_bucket_public_access_block" "puep_bucket_allow_public_access" {
      ...
      "Statement": [
        {
          ...
          "Condition": {
            "StringEquals": {
              "AWS:SourceArn": "arn:aws:cloudfront::385526948728:distribution/E3SHTYQ25S0OP8"
            }
          }
        }
      ]
...
resource "aws_s3_bucket_public_access_block" "puep_bucket_block_public_access" {
  bucket = aws_s3_bucket.puep_website_s3_bucket.id
